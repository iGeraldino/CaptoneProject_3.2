...
@section('content')
   ...
   $clearBqtSession_Value = Session::get('AdminBqtClearSession');
   Session::remove('AdminBqtClearSession');


   $clearCartSession_Value = Session::get('CartClearSession');
   Session::remove('CartClearSession');

  $sessionValue = Session::get('Added_FlowerToAdminBQT_Order');
  Session::remove('Added_FlowerToAdminBQT_Order');//determines the addition of new flower

  $sessionAcValue = Session::get('Added_AcessoryToAdminBQT_Order');
  Session::remove('Added_AcessoryToAdminBQT_Order');//determines the addition of new acessory

  $sessionUpdateFValue = Session::get('Update_FlowerToAdminBQT_Order');
  Session::remove('Update_FlowerToAdminBQT_Order');//deteremines the qty update of flower*/

  $sessionUpdateAcValue = Session::get('Update_AcessoryToAdminBQT_Order');
  Session::remove('Update_AcessoryToAdminBQT_Order');//deteremines the qty update of acessories*/

  $sessionDelFlowerValue = Session::get('Deleted_FlowerfromAdminBQT_Order');
  Session::remove('Deleted_FlowerfromAdminBQT_Order');//determines the deletion of flower

  $sessionDelAcessoryValue = Session::get('Deleted_AcessoryfromAdminBQT');
  Session::remove('Deleted_AcessoryfromAdminBQT');//determines the deletion of Acessory
  ...
  @foreach(Cart::instance('AdminBqt_Flowers')->content() as $Flowersrow)
  <?php
		...
		<div class="row">
			<div class="col-md-6" style="margin-left: -7px;">
				<div class="title container" style="margin-top: 3%;">
					...
					<h4>Have Fun making Bouquets</h4>
          <h7>Bouquets made here will be posted at the website</h7>
					<h7>which will be seen by online customers</h7>
				</div>
					...
					<div class="panel">
						<div class="panel">
                    		...
                    		<div class="panel-body">
                  			...
                  			@foreach(Cart::instance('AdminBqt_Flowers')->content() as $BQT_Flowers)
                  {!! Form::model($BQT_Flowers, ['route'=>['bouqAdd_Flower_toAdmin_Bqt.update', $BQT_Flowers->id],'method'=>'PUT'])!!}
                    			<div class="row">
                              ...
                              <div class="col-xs-3" style = "color:darkviolet; margin-top:3%;">
			                        ...
			                        <div class="col-xs-1">
			                        	<a class="btn Love btn-just-icon" href ="{{ route('AdminBqtFlowerorderSessions.DelOrderFlowers',['flower_ID'=>$BQT_Flowers->id]) }}" data-toggle="tooltip" title="Delete">
							...
							{!! Form::close() !!}
                  			@endforeach
                    			<!---->
		                @foreach(Cart::instance('AdminBqt_Acessories')->content() as $BQT_Acessories)
<!--form model here-->
                      {!! Form::model($BQT_Acessories, ['route'=>['bouqAdd_Acrs_toAdmin_Bqt.update', $BQT_Acessories->id],'method'=>'PUT'])!!}
                    			<div class="row">
                              ...
                              <div class="col-xs-3" style = "color:darkviolet; margin-top:3%;">
                                <h7><b>=</b> Php {{number_format($BQT_Acessories->price * $BQT_Acessories->qty,2)}}</h7>
			                        ...
			                        <div class="col-xs-1">
			                        	<a class="btn Love btn-just-icon" href ="{{ route('Admin.DelAcessories',['Acessory_ID'=>$BQT_Acessories->id]) }}" data-toggle="tooltip" title="Delete">
            											<i class="material-icons">delete</i>
            										</a>
			                        </div>
                      			</div>
...
<!--form close here-->
                          ...
                          <a  id = "saveBtn" class="btn btn-sm Lemon" data-toggle="modal" data-target="#Final_modal"> Create Bouquet</a>

                    			<a  href = "{{route('Order.ClearAdminBqt')}}" type="button" class="btn Love btn-sm" data-toggle="tooltip" data-placement="top" title="This Button will reset the progress of your creation" data-container="body"> Clear Bouquet</a>
                    		</div>
						</div>
					</div>
				</div>
  ...
  <!--MODAL Bouquet Final-->
	...
	<!-- Modal Core -->
	<div class="modal fade" id="Final_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
<!--form open here-->
{!! Form::open(array('route' => 'bouquet.store', 'data-parsley-validate'=>'', 'files' => 'true', 'method'=>'POST')) !!}
	      	<div class="modal-header">
			        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
			        <h4 class="modal-title text-center" id="myModalLabel"><b>Final requirement:</b></h4>
			    </div>
	    		<div class="modal-body">
            <div class="form-group" Style = "margin-left: 31%;">
              <img src= "{{ asset('img/'.'addfile.ico')}}"  class = "img-rounded img-responsive img-raised" id="imageBox" name="imageBox" style="max-width: 200px; max-height: 200px;" />
            </div>
            <div class="input-group">
              <input class ="uploader" type="file" accept="image/*" name = "bouimg" id = "bouimg" onchange="preview_image(event)" style = "margin-top: 2%;" required>
            </div>
	    		</div>

	    		<div class="modal-footer">
			        <button type="button" class="btn btn-danger btn-simple" data-dismiss="modal">Cancel</button>
			        <button type="submit" class="btn btn-success btn-simple">Process Creation</button>
			    </div>
          {!! Form::close() !!}
          <!--form close here-->
	    	</div>
	...
	<div class="modal fade" id="Bqtflower_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
<!--form open here-->
{!! Form::open(array('route' => 'bouqAdd_Flower_toAdmin_Bqt.store', 'data-parsley-validate'=>'', 'method'=>'POST')) !!}
	      	<div class="modal-header">
			        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
...
{!! Form::close() !!}
	...
	<div class="modal fade" id="accessoriesmodal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
	    	<div class="modal-content">
<!--form open here-->
{!! Form::open(array('route' => 'bouqAdd_Acrs_toAdmin_Bqt.store', 'data-parsley-validate'=>'', 'files' => 'true', 'method'=>'POST')) !!}
	      		<div class="modal-header">
			        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
...
@section('scripts')
  ...
  $(document).ready(function(){
     ...
     if($('#UpdateFlower_result').val()=='NoChange'){
 	    //Show popup
 	    swal("Oops!","Flower that you tried to change quantity seemd to be not in the progress, please try again","warning");
 	   }
